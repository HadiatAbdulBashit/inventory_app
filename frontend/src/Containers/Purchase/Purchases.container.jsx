...
const Purchases = () => {
  ...
  const getTransaction = async () => {
    const response = await axios.get(`/api/transaction?page=${page}&search=${search}&sort=${sort.sort},${sort.order}&type=In&limit=${limit}&status=${filter}` + (filterDate === 'month' ? `&month=${monthFilter.split('-')[1]}&year=${monthFilter.split('-')[0]}` : filterDate === 'range' ? `&startDate=${startDate}&endDate=${endDate}` : ''));
        ...
        <div className="panel-heading">
          <div className="d-flex justify-content-between">
            <div className="d-flex">
              {
                user.role === 'Office' ? (
                  <Link to="/dashboard/purchase/add" className="btn btn-primary me-2">
                    Add New Purchase
                  </Link>
                ) : null
              }
